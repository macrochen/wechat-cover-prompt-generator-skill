...
3. **内容要求**：文件内容应直接由绘图描述、视觉元素和文字说明组成。
4. **比例强制**：必须在提示词末尾或显著位置明确标注比例（例如：`--ar 2.35:1` 或 `比例 2.35:1`）。微信公众号封面默认使用 **2.35:1**。
5. **易用性**：确保提示词可以直接被复制到 AI 绘图工具（如 Midjourney, DALL-E, Gemini）中使用。
